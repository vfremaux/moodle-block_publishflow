...
function publishflow_session_open($course, $notify) {
    ...
    if ($course->category == @$config->coursedelivery_deploycategory || $course->category == @$config->coursedelivery_closedcategory) {
        $course->category = @$config->coursedelivery_runningcategory;
    }
...
function publishflow_course_close($course, $mode, $rpccall = false) {
    ...
    if (empty($config->coursedelivery_closedcategory)) {
        if (!$rpccall) {
    ...
    $context = context_course::instance($course->id);
    if ($course->category == @$config->coursedelivery_runningcategory || $course->category == @$config->coursedelivery_deploycategory) {
        $course->category = $config->coursedelivery_closedcategory;
    }
...
function publishflow_local_deploy($category, $sourcecourse) {
    ...
    if (!$category) {
        $category = $config->coursedelivery_deploycategory;
    }
...
function block_build_trainingcenter_menu($block) {
        ...
        if (empty($factoryhost)) {
            ...
            if (empty($realpath)) {
                ...
                if (has_capability('block/publishflow:retrofit', $context_course) || block_publishflow_extra_retrofit_check()){
                    $retrofiturl = new moodle_url('/blocks/publishflow/retrofit.php', array('fromcourse' => $COURSE->id, 'what' => 'retrofit', 'where' => $factoryhost->id));
    ...
    if (has_capability('block/publishflow:manage', $context_course)) {
        ...
        if ($COURSE->category == $config->coursedelivery_runningcategory && $COURSE->visible) {
        ...
        } else if ($COURSE->category == $config->coursedelivery_deploycategory) {
            $openurl = new moodle_url('/blocks/publishflow/open.php', array('fromcourse' => $COURSE->id, 'what' => 'open'));
            $output .= '<a href="'.$openurl.'">'.$stropen.'</a>';
        } else if ($COURSE->category == $config->coursedelivery_closedcategory) {
            $openurl = new moodle_url('/blocks/publishflow/open.php', array('fromcourse' => $COURSE->id, 'what' => 'open'));
