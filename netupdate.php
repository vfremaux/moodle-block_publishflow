...
$params = array('section' => 'blocksettingpublishflow');
$PAGE->navbar->add(get_string('setup', 'block_publishflow'), new moodle_url('/admin/settings.php', $params));
...
$hosts = $DB->get_records_select('mnet_host', " deleted = 0 AND wwwroot != ? ", array($CFG->wwwroot));
...
if (!($errors == 0)) {
    $errorstr = get_string('erroradvice', 'block_publishflow', $errors);
    echo $OUTPUT->box(get_string('errormonitoring', 'block_publishflow').'<br/>'.$errorstr);
} else if (!($warnings == 0)) {
    $warningstr = get_string('warningadvice', 'block_publishflow', $warnings);
    echo $OUTPUT->box(get_string('errormonitoring', 'block_publishflow').'<br/>'.$warningstr);
} else {
