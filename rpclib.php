...
function publishflow_rpc_deploy($callinguser, $idfield, $courseidentifier, $whereroot, $parmsoverride = null, $json_response = true){
    ...
    if (!empty($whereroot)){
	    if (!$mnethost = get_record('mnet_host', 'wwwroot', $whereroot, 'deleted', 0)){
    ...
    if (!empty($USER->mnethostid)){
        if ($userhost = get_record('mnet_host', 'id', $USER->mnethostid, 'deleted', 0)){
...
function publishflow_updateplatforms($callinguser, $platformroot){
	...
	$records = get_records('course_categories','visible','1','','name,id,parent');

	$response = new stdclass;
	if($nodetype == ''){
		$responses->status = RPC_FAILURE;
		$response->errors[] = 'Incompatible Platform';
		$response->error = 'Incompatible Platform';
	} else {
		$response->status = RPC_SUCCESS;
		$response->errors = array();
		$response->error = '';
...
*/
function delivery_get_sessions($callinguser, $course_idnumber, $json_response = true){
    global $CFG;
    ...
    if ($CFG->coursedelivery_publicsessions != COURSESESSIONS_PUBLIC){
        ...
        if (!$auth_response){
	        $candeploy = has_capability('block/publishflow:deploy', get_context_instance(CONTEXT_SYSTEM), $USER->id);
	        $canpublish = has_capability('block/publishflow:publish', get_context_instance(CONTEXT_SYSTEM), $USER->id);
	    }
    }    
     
    // get all courses that are instances of the given idnumber LP identifier
    $sessions = get_records('course', 'idnumber', $course_idnumber, 'shortname', 'id,shortname,fullname,startdate,visible');
    if (!empty($sessions)){
        foreach($sessions as $session){
            $context = get_context_instance(CONTEXT_COURSE, $session->id);
...
function delivery_deliver($callinguser, $lp_catalogcourseid, $transferoffset = 0, $transferlimit = 0, $json_response = true){
    ...
    if (!$course = get_record('course', 'id', $lp_catalogcourseid)){
        $response->status = RPC_FAILURE;
    ...
    $realpath = delivery_check_available_backup($lp_catalogcourseid, $loopback);
    ...
    if (empty($realpath)){
        $response->status = RPC_FAILURE;
        $response->errors[] = 'No available deployable backup for this course '. ' ' . $realpath->dir;
        $response->error = 'No available deployable backup for this course '. ' ' . $realpath->dir;
        return publishflow_send_response($response, $json_response);
    } else {    
        ...
        if (file_exists($realpath->path)){
            if (empty($CFG->coursedeliveryislocal)){
                ...
                backup_file2data($realpath->path, $filecontent);
                $response->archivename = $realpath->path;
...
function delivery_deploy($callinguser, $sourcecourseserial, $forcereplace, $parmsoverride = null, $json_response = true){
    ...
    if (!isset($CFG->coursedelivery_coursefordelivery) || !$course = get_record('course', 'id', $CFG->coursedelivery_coursefordelivery)){
    ...
    if (!file_exists($realpath) || $forcereplace){
    ...
    if (empty($parmsoverride['category'])){    
        $deploycat = get_record('course_categories', 'id', @$CFG->coursedelivery_deploycategory);
    } else {
        $deploycat = get_record('course_categories', 'id', $parmsoverride['category']);
    }
	...
	debug_trace('DEPLOY : Up to deploy course. We have everything in '.$realpath);
    ...
    set_field('course', 'idnumber', $sourcecourse->idnumber, 'id', "{$response->courseid}");

    // confirm/force guest closure
    set_field('course', 'guest', 0, 'id', "{$response->courseid}");

    // confirm/force not enrollable // enrolling will be performed by master teacher
    set_field('course', 'enrollable', 0, 'id', "{$response->courseid}");
    ...
    if (!empty($CFG->coursedelivery_defaultrole)){
	    $coursecontext = get_context_instance(CONTEXT_COURSE, $response->courseid);
...
function delivery_publish($callinguser, $action, $sourcecourseserial, $forcereplace, $json_response = true){
    ...
    switch($action){
        case 'unpublish' : {
            set_field('course', 'visible', 0, 'idnumber', $sourcecourse->idnumber); 
            $response->status = RPC_SUCCESS;
            $response->username = $username;
            $response->remotehostroot = $remotehostroot;
            $response->courseid = get_field('course', 'id', 'idnumber', $sourcecourse->idnumber);
            return publishflow_send_response($response, $json_response);
        }
        break;
        case 'publish' : {
            ...
            $courseexists = count_records('course', 'idnumber', $sourcecourse->idnumber);
            if (!$forcereplace && $courseexists){
                
                // we do not need anything else than making this course(s) visible back.
                set_field('course', 'visible', 1, 'idnumber', $sourcecourse->idnumber); 
                $response->status = RPC_SUCCESS;
                $response->username = $username;
                $response->remotehostroot = $remotehostroot;
            ...
            set_field('course', 'guest', 1, 'id', "{$response->courseid}");
            ...
            if (!isset($sourcecourse->retrofit)){
                ...
                set_field('course', 'approval_status_id', COURSE_STATUS_PUBLISHED, 'id', "{$response->courseid}");
            } else {
                // reset to unsubmitted course if we are backfeeding the factory
                set_field('course', 'approval_status_id', COURSE_STATUS_NOTSUBMITTED, 'id', "{$response->courseid}");
            }
            */
